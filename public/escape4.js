...
var playMode=[3,'クロンダイク','スパイダー','エルドリッチ']
var cards = [];
var hands = [];
...
function monsterMove(){
  ...
  for(var i=0;i<4;i++){
    if(Extras[i]!==-1 && attacker[i][0]!==-1 && attacker[i][1]!==-1){
      ...
      //2 スートが同じエースは7としても扱う
      var AA=Math.floor((Extras[i])/13);
      var BB=Math.floor((attacker[i][0])/13);
      var CC=Math.floor((attacker[i][0])/13);
      if(B ==1 && AA==BB){
        if(A==7+C){
          //破壊
          Destraction(i);
